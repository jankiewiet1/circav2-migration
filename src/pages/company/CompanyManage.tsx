...
export default function CompanyManage() {
  ...
  useEffect(() => {
    if (!company?.id) {
      console.log('CompanyManage: No company ID available yet');
      return;
    }

    const fetchMembers = async () => {
      console.log('CompanyManage: Starting fetchMembers for company ID:', company.id);
      setLoadingMembers(true);
      try {
        console.log('CompanyManage: Fetching members for company ID:', company.id);
        const members = await getCompanyMembers(company.id);
        console.log('CompanyManage: Received members:', members);
        console.log('CompanyManage: Number of members:', members.length);
        ...
        setCompanyMembers([]); // Clear on error
      } finally {
  ...
  const handleCreateInvitation = async (email: string, role: UserRole) => {
    try {
      ...
      // Use the new team invite service with email functionality
                  ...
                  <div className="flex justify-end">
                    <Button 
                      ...
                      onClick={async () => {
                        ...
                        setLoadingMembers(true);
                        try {
                          console.log('Manual refresh: Fetching members for company ID:', company.id);
                          const members = await getCompanyMembers(company.id);
                          console.log('Manual refresh: Received members:', members);
                          setCompanyMembers(members);
                          toast.success(`Refreshed: Found ${members.length} members`);
                        } catch (error) {
